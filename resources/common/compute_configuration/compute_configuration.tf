...
resource "null_resource" "prepare_ansible_inventory_using_jumphost_connection" {
  ...
  provisioner "local-exec" {
    ...
    command     = "python3 ${local.ansible_inv_script_path} --tf_inv_path ${var.inventory_path} --install_infra_path ${var.clone_path} --instance_private_key ${local.compute_private_key} --bastion_user ${var.bastion_user} --bastion_ip ${var.bastion_instance_public_ip} --bastion_ssh_private_key ${var.bastion_ssh_private_key} --memory_size ${var.memory_size} --max_pagepool_gb ${var.max_pagepool_gb} --using_packer_image ${var.using_packer_image} --using_rest_initialization ${var.using_rest_initialization} --gui_username ${var.compute_cluster_gui_username} --gui_password ${var.compute_cluster_gui_password} --enable_mrot_conf ${var.enable_mrot_conf} --config_ces ${var.config_ces} --client_instance_private_key '' --ldap_basedns ${var.ldap_basedns}"
  }
...
resource "null_resource" "prepare_ansible_inventory_using_jumphost_connection_encryption" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "python3 ${local.ansible_inv_script_path} --tf_inv_path ${var.inventory_path} --install_infra_path ${var.clone_path} --instance_private_key ${local.compute_private_key} --bastion_user ${var.bastion_user} --bastion_ip ${var.bastion_instance_public_ip} --bastion_ssh_private_key ${var.bastion_ssh_private_key} --memory_size ${var.memory_size} --max_pagepool_gb ${var.max_pagepool_gb} --using_packer_image ${var.using_packer_image} --using_rest_initialization ${var.using_rest_initialization} --gui_username ${var.compute_cluster_gui_username} --gui_password ${var.compute_cluster_gui_password} --enable_mrot_conf ${var.enable_mrot_conf} --config_ces ${var.config_ces} --scale_encryption_enabled ${var.scale_encryption_enabled} --scale_encryption_servers ${local.scale_encryption_servers} --scale_encryption_admin_password ${var.scale_encryption_admin_password} --client_instance_private_key '' --ldap_basedns ${var.ldap_basedns}"
  }
  depends_on = [local_file.create_compute_tuning_parameters, local_sensitive_file.write_meta_private_key]
  triggers = {
    build = timestamp()
...
resource "null_resource" "prepare_ansible_inventory" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "python3 ${local.ansible_inv_script_path} --tf_inv_path ${var.inventory_path} --install_infra_path ${var.clone_path} --instance_private_key ${local.compute_private_key} --memory_size ${var.memory_size} --max_pagepool_gb ${var.max_pagepool_gb} --using_packer_image ${var.using_packer_image} --using_rest_initialization ${var.using_rest_initialization} --gui_username ${var.compute_cluster_gui_username} --gui_password ${var.compute_cluster_gui_password} --enable_mrot_conf ${var.enable_mrot_conf} --config_ces ${var.config_ces} --client_instance_private_key '' --ldap_basedns ${var.ldap_basedns}"
  }
  depends_on = [local_file.create_compute_tuning_parameters, local_sensitive_file.write_meta_private_key]
  triggers = {
    build = timestamp()
...
resource "null_resource" "prepare_ansible_inventory_encryption" {
  ...
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command     = "python3 ${local.ansible_inv_script_path} --tf_inv_path ${var.inventory_path} --install_infra_path ${var.clone_path} --instance_private_key ${local.compute_private_key} --memory_size ${var.memory_size} --max_pagepool_gb ${var.max_pagepool_gb} --using_packer_image ${var.using_packer_image} --using_rest_initialization ${var.using_rest_initialization} --gui_username ${var.compute_cluster_gui_username} --gui_password ${var.compute_cluster_gui_password} --enable_mrot_conf ${var.enable_mrot_conf} --config_ces ${var.config_ces} --scale_encryption_enabled ${var.scale_encryption_enabled} --scale_encryption_servers ${local.scale_encryption_servers} --scale_encryption_admin_password ${var.scale_encryption_admin_password} --client_instance_private_key '' --ldap_basedns ${var.ldap_basedns}"
  }
  depends_on = [local_file.create_compute_tuning_parameters, local_sensitive_file.write_meta_private_key]
  triggers = {
    build = timestamp()
