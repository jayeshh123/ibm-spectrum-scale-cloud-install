...
locals {
  ...
  disk_count           = data.ibm_is_bare_metal_server_profile.itself.disks[1].quantity[0].value
...
output "instance_ips_with_vol_mapping" {
  value      = try({ for instance_details in ibm_is_bare_metal_server.itself : instance_details.name => local.selected_disks }, {})
  depends_on = [ibm_dns_resource_record.a_itself, ibm_dns_resource_record.ptr_itself]
}
